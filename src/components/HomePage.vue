<script setup>
		...
		if (!store.products.length) {
			...
			const getAndSetProducts = async () => {
				try {
					...
					productsToRender.value = [...res.data.products].slice((page.value - 1) * 12, page.value * 12)
	...
	watch(search, () => {
		filteredProducts.value = [...store.products]
			.filter(product => product.title.toLowerCase().includes(search.value.toLowerCase()));
	...
	watch(sortBy, () => {
		switch(sortBy.value) {
			...
			case 'By title':
				filteredProducts.value = [...filteredProducts.value].sort((a, b) => a.title.localeCompare(b.title))
				break;
